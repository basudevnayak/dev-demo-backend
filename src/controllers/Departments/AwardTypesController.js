...
const AwardTypesController = {
    ...
    async update(req, res, next) {
        ...
        try {
            ...
            const existingDepartment = await AwardTypes.findOne({
                name: req.body.name,
            ...
            const updatedDepartment = await AwardTypes.findByIdAndUpdate(
                id,
                {
                    $set: {
                        ...req.body,
    ...
    async destroy(req, res, next) {
        try {
            ...
            const result = await AwardTypes.deleteMany({ _id: { $in: ids } });
    ...
    async index(req, res, next) {
        try {
            ...
            const [documents, total] = await Promise.all([
                AwardTypes.find(filter)
                    .select('-__v -updatedAt')
                    .sort(sort)
                    .skip(skip)
                    .limit(limitNum),
                AwardTypes.countDocuments(filter)
            ]);
            ...
            const list = documents.map((_data) => ({
                ..._data._doc,
                ..._data,
                name: AwardTypesController.capitalize(_data.name), // Capitalize the name
                remark: _data.remark,
                id: _data._id,
            }));
    ...
    async show(req, res, next) {
        let document;
        try {
            document = await AwardTypes.findOne({ _id: req.params.id }).select(
                '-updatedAt -__v'
            );
    ...
    async import(req, res, next) {
        try {
            ...
            const existingDepartments = await AwardTypes.find({
            ...
            const savedDepartments = await AwardTypes.insertMany(newDepartments);
            return res.status(200).json({
                status: 200,
    ...
    async export(req, res, next) {
        try {
            const departments = await AwardTypes.find({})
...
export default AwardTypesController;
