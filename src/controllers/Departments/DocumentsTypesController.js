import { DocumentsTypes } from '../../models/index.js';
import path from 'path';
import CustomErrorHandler from '../../utils/CustomErrorHandler.js';
import fs from 'fs';
import Joi from 'joi';
const DocumentsTypesController  = {
    capitalize(str) {
    ...
    async store(req, res, next) {
        ...
        try {
            const exists = await DocumentsTypes.findOne({ name });
            if (exists) {
            ...
            const department = new DocumentsTypes({
                name: name,
                remark,
                updatedAt: new Date(),
            });
    ...
    async update(req, res, next) {
        ...
        try {
            ...
            const existingDepartment = await DocumentsTypes.findOne({
                name: req.body.name,
            ...
            const updatedDepartment = await DocumentsTypes.findByIdAndUpdate(
                id,
                {
                    $set: {
                        ...req.body,
    ...
    async destroy(req, res, next) {
        try {
            ...
            const result = await DocumentsTypes.deleteMany({ _id: { $in: ids } });
    ...
    async index(req, res, next) {
        try {
            ...
            const [documents, total] = await Promise.all([
                DocumentsTypes.find(filter)
                    .select('-__v -updatedAt')
                    .sort(sort)
                    .skip(skip)
                    .limit(limitNum),
                DocumentsTypes.countDocuments(filter)
            ]);
            ...
            const list = documents.map((_data) => ({
                ..._data._doc,
                ..._data,
                name: DocumentsTypesController.capitalize(_data.name), // Capitalize the name
                remark: _data.remark,
                id: _data._id,
            }));
    ...
    async show(req, res, next) {
        let document;
        try {
            document = await DocumentsTypes.findOne({ _id: req.params.id }).select(
                '-updatedAt -__v'
            );
    ...
    async import(req, res, next) {
        try {
            ...
            const existingDepartments = await DocumentsTypes.find({
            ...
            const savedDepartments = await DocumentsTypes.insertMany(newDepartments);
            return res.status(200).json({
                status: 200,
    ...
    async export(req, res, next) {
        try {
            const departments = await DocumentsTypes.find({})
