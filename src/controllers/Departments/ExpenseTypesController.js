import { ExpenseTypes } from '../../models/index.js';
import path from 'path';
import CustomErrorHandler from '../../utils/CustomErrorHandler.js';
import fs from 'fs';
import Joi from 'joi';
const ExpenseTypesController  = {
    capitalize(str) {
    ...
    async store(req, res, next) {
        ...
        try {
            const exists = await ExpenseTypes.findOne({ name });
            if (exists) {
            ...
            const department = new ExpenseTypes({
                name: name,
                remark,
                updatedAt: new Date(),
            });
    ...
    async update(req, res, next) {
        ...
        try {
            ...
            const existingDepartment = await ExpenseTypes.findOne({
                name: req.body.name,
            ...
            const updatedDepartment = await ExpenseTypes.findByIdAndUpdate(
                id,
                {
                    $set: {
                        ...req.body,
    ...
    async destroy(req, res, next) {
        try {
            ...
            const result = await ExpenseTypes.deleteMany({ _id: { $in: ids } });
    ...
    async index(req, res, next) {
        try {
            ...
            const [documents, total] = await Promise.all([
                ExpenseTypes.find(filter)
                    .select('-__v -updatedAt')
                    .sort(sort)
                    .skip(skip)
                    .limit(limitNum),
                ExpenseTypes.countDocuments(filter)
            ]);
            ...
            const list = documents.map((_data) => ({
                ..._data._doc,
                ..._data,
                name: ExpenseTypesController.capitalize(_data.name), // Capitalize the name
                remark: _data.remark,
                id: _data._id,
            }));
    ...
    async show(req, res, next) {
        let document;
        try {
            document = await ExpenseTypes.findOne({ _id: req.params.id }).select(
                '-updatedAt -__v'
            );
    ...
    async import(req, res, next) {
        try {
            ...
            const existingDepartments = await ExpenseTypes.find({
            ...
            const savedDepartments = await ExpenseTypes.insertMany(newDepartments);
            return res.status(200).json({
                status: 200,
    ...
    async export(req, res, next) {
        try {
            const departments = await ExpenseTypes.find({})
...
export default ExpenseTypesController ;
