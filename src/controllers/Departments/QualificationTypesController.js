import { QualificationTypes } from '../../models/index.js';
import path from 'path';
import CustomErrorHandler from '../../utils/CustomErrorHandler.js';
import fs from 'fs';
import Joi from 'joi';
const QualificationTypesController = {
    capitalize(str) {
    ...
    async store(req, res, next) {
        ...
        try {
            const exists = await QualificationTypes.findOne({ name });
            if (exists) {
            ...
            const department = new QualificationTypes({
                name: name,
                remark,
                updatedAt: new Date(),
            });
    ...
    async update(req, res, next) {
        ...
        try {
            ...
            const existingDepartment = await QualificationTypes.findOne({
                name: req.body.name,
            ...
            const updatedDepartment = await QualificationTypes.findByIdAndUpdate(
                id,
                {
                    $set: {
                        ...req.body,
    ...
    async destroy(req, res, next) {
        try {
            ...
            const result = await QualificationTypes.deleteMany({ _id: { $in: ids } });
    ...
    async index(req, res, next) {
        try {
            ...
            const [documents, total] = await Promise.all([
                QualificationTypes.find(filter)
                    .select('-__v -updatedAt')
                    .sort(sort)
                    .skip(skip)
                    .limit(limitNum),
                QualificationTypes.countDocuments(filter)
            ]);
            ...
            const list = documents.map((_data) => ({
                ..._data._doc,
                ..._data,
                name: QualificationTypesController.capitalize(_data.name), // Capitalize the name
                remark: _data.remark,
                id: _data._id,
            }));
    ...
    async show(req, res, next) {
        let document;
        try {
            document = await QualificationTypes.findOne({ _id: req.params.id }).select(
                '-updatedAt -__v'
            );
    ...
    async import(req, res, next) {
        try {
            ...
            const existingDepartments = await QualificationTypes.find({
            ...
            const savedDepartments = await QualificationTypes.insertMany(newDepartments);
            return res.status(200).json({
                status: 200,
    ...
    async export(req, res, next) {
        try {
            const departments = await QualificationTypes.find({})
...
export default QualificationTypesController;
