...
const OrganizationController = {
    ...
    async update(req, res, next) {
        ...
        try {
            ...
            const existingDepartment = await ClientGroup.findOne({
                GroupName
                // GroupCode, Country, Remark,
            ...
            const updatedDepartment = await ClientGroup.findByIdAndUpdate(
                id,
                {
                    $set: {
                        ...req.body,
    ...
    async destroy(req, res, next) {
        try {
            ...
            const result = await ClientGroup.deleteMany({ _id: { $in: ids } });
    ...
    async index(req, res, next) {
        try {
            ...
            const [documents, total] = await Promise.all([
                ClientGroup.find(filter)
                    .select('-__v -updatedAt')
                    .sort(sort)
                    .skip(skip)
                    .limit(limitNum),
                ClientGroup.countDocuments(filter)
            ]);
    ...
    async show(req, res, next) {
        let document;
        try {
            document = await ClientGroup.findOne({ _id: req.params.id }).select(
                '-updatedAt -__v'
            );
    ...
    async import(req, res, next) {
        try {
            ...
            const existingDepartments = await ClientGroup.find({
            ...
            const savedDepartments = await ClientGroup.insertMany(newDepartments);
            return res.status(200).json({
                status: 200,
    ...
    async export(req, res, next) {
        try {
            const departments = await ClientGroup.find({})
